...
public class PacienteController {
    ...
    public ResponseEntity<MensajeDTO<String>> registrarse(@Valid @RequestBody RegistroPacienterDTO registroPacienteDTO)
            throws Exception {
    ...
    public ResponseEntity<MensajeDTO<String>> editarPerfil(@PathVariable int codigo,
            @Valid @RequestBody DetallePacienteDTO detallePacienteDTO) throws Exception {
    ...
    public ResponseEntity<MensajeDTO<DetallePacienteDTO>> verDetallePaciente(@PathVariable int codigo)
            throws Exception {
    ...
    public ResponseEntity<MensajeDTO<List<ItemPqrsDTO>>> listarPqrsPaciente(@PathVariable int idPaciente)
            throws Exception {
    ...
    public ResponseEntity<MensajeDTO<List<ItemCitaPacienteDTO>>> listarCitasPaciente(@PathVariable int codigoPaciente)
            throws Exception {
    ...
    public ResponseEntity<MensajeDTO<List<ItemCitaDTO>>> filtrarCitasPorFecha(FiltrarSearchCitaDTO filtrarSearchCitaDTO)
            throws Exception {
        return ResponseEntity.ok()
                .body(new MensajeDTO<>(false, pacienteServicio.filtrarCitasPorFecha(filtrarSearchCitaDTO)));
    }

    @GetMapping("/listar-citas-medico")
    public ResponseEntity<MensajeDTO<List<ItemMedicoCitaDTO>>> filtrarMedicoCita(
            @RequestParam Especialidad especialidad,
            @RequestParam LocalDateTime fecha) throws Exception {
        return ResponseEntity.ok()
                .body(new MensajeDTO<>(false, pacienteServicio.filtrarMedicoCita(especialidad, fecha)));
    }

    @PostMapping("/agendar-cita")
    public ResponseEntity<MensajeDTO<String>> agendarCita(@Valid @RequestBody CitaPacienteDTO citaDTO)
            throws Exception {
    ...
    public ResponseEntity<MensajeDTO<String>> crearPQRS(@Valid @RequestBody PqrsPacienteDTO pqrsPacienteDTO)
            throws Exception {
        pacienteServicio.crearPQRS(pqrsPacienteDTO);
        return ResponseEntity.ok().body(new MensajeDTO<>(false, "PQRS creada exitosamente"));
    }

    @PostMapping("/responder-pqrs")
    public ResponseEntity<MensajeDTO<String>> responderPQRS(
            @Valid @RequestBody RespuestaPacientePqrsDTO respuestaPacientePqrsDTO) throws Exception {
        pacienteServicio.responderPQRS(respuestaPacientePqrsDTO);
        return ResponseEntity.ok().body(new MensajeDTO<>(false, "Respuesta enviada exitosamente"));
    }
    ...
    public ResponseEntity<MensajeDTO<String>> enviarLinkRecuperacion(@RequestParam String correo) throws Exception {
        ...
        return ResponseEntity.ok().body(new MensajeDTO<>(false, "Link de recuperación enviado"));
    }
