...
public class PacienteServicioImpl implements PacienteServicio {
    ...
    public int editarPerfil(int codigo, DetallePacienteDTO detallePacienteDTO) throws Exception {
        ...
        if (estaRepetidoCorreo(detallePacienteDTO.correo())
                && (!buscado.getCorreo().equals(detallePacienteDTO.correo()))) {
            throw new Exception("El correo ya se encuentra registrado");
        }
        if (estaRepetidaCedula(detallePacienteDTO.cedula())
                && (!buscado.getCedula().equals(detallePacienteDTO.cedula()))) {
    ...
    public int agendarCita(CitaPacienteDTO citaDTO) throws Exception {
        ...
        EstadoCita estadoPendiente = estadoCitaRepo.findById(1)
                .orElseThrow(() -> new Exception("Estado PENDIENTE no encontrado")); // Assuming 1 is PENDIENTE
        long numeroCitasActivas = citaRepo.countAllByPacienteIdAndEstadoCita(citaDTO.codigoPaciente(), estadoPendiente);
        ...
        citaNueva.setEstadoCita(estadoPendiente);

        Cita citaRegistrada = citaRepo.save(citaNueva);

        emailServicio.EnviarEmail(new EmailDTO("Agendamiento de Cita", paciente.get().getCorreo(),
                "Haz agendado una cita para el " + citaRegistrada.getFechaCita() + " Motivo: "
                        + citaRegistrada.getMotivo() + " con el especialista en "
                        + citaRegistrada.getMedico().getEspecialidad().getNombre() + " "
                        + citaRegistrada.getMedico().getNombre()));

        emailServicio.EnviarEmail(new EmailDTO("Agendamiento de Cita", medico.get().getCorreo(),
                "Se ha agendada " + citaRegistrada.getFechaCita() + " Motivo: " + citaRegistrada.getMotivo()
                        + " Nombre del Paciente: " + paciente.get().getNombre()));
    ...
    public int crearPQRS(PqrsPacienteDTO pqrsPacienteDTO) throws Exception {
        ...
        EstadoPqrs estadoNuevo = estadoPqrsRepo.findById(1)
                .orElseThrow(() -> new Exception("Estado NUEVO no encontrado")); // Assuming 1 is NUEVO
        List<Pqrs> pqrsList = pqrsRepo
                .findAllByCita_Paciente_IdAndEstadoPqrsEquals(opcional.get().getPaciente().getCodigo(), estadoNuevo);
        ...
        pqrsNew.setEstadoPqrs(estadoNuevo);
    ...
    public int responderPQRS(RespuestaPacientePqrsDTO respuestaPacientePqrsDTO) throws Exception {
        ...
        // Assuming respuestaAdmin is the ID of the previous message we are replying to,
        // but Mensaje structure is flat list for PQRS.
        // Or maybe it's just a new message.
        // The DTO has `respuestaAdmin` field. Let's check if we need it.
        // If it's just adding a message to the PQRS, we don't strictly need the parent
        // message unless we want threading.
        // But the previous logic was: find RespuestaAdmin, then create
        // RespuestaPaciente linking to it.
        // New logic: Create Mensaje linked to PQRS and Account (Paciente).
        ...
        if (opcionalPqrs.get().getEstadoPqrs().equals(estadoEnProceso)) {
            ...
            mensaje.setMensaje(respuestaPacientePqrsDTO.mensaje());
            mensaje.setCuenta(buscadoPaciente); // Paciente is a Cuenta

            Mensaje mensajeRegistrado = mensajeRepo.save(mensaje);

            // Email logic might need adjustment if we don't have the admin email readily
            // available from a previous message.
            // But we can send to admins or just skip for now if we don't know which admin
            // to reply to.
            // Or maybe we send to all admins? Or the one who replied last?
            // For now, let's just return the ID.

            return mensajeRegistrado.getCodigo();

        } else {
            throw new Exception("Su estado es: " + opcionalPqrs.get().getEstadoPqrs().getEstado()
                    + " por lo tanto no es posible una respuesta");
        }
    }
    ...
    @Override
    public List<ItemMedicoCitaDTO> filtrarMedicoCita(Especialidad especialidad, LocalDateTime fecha) throws Exception {
        ...
        int numeroDia = dayWeek.getValue();
        Dia dia = diaRepo.findById(numeroDia).orElseThrow(() -> new Exception("Dia no encontrado: " + numeroDia));
    ...
    public List<ItemCitaDTO> filtrarCitasPorFecha(FiltrarSearchCitaDTO filtrarSearchCitaDTO) throws Exception {
        ...
        if (filtrarSearchCitaDTO.fechaCita() != null) {
            citas = citaRepo.findCitasCompletadasByPacienteAndNombreMedicoAndFechaCita(codigoPaciente, nombreMedico,
                    fechaCita);
