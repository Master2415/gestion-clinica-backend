...
public class PacienteServicioTest {
    ...
    public void registrarTest() throws Exception {
        ...
        // Fetch entities from DB (assuming dataset.sql populates them)
        ...
        RegistroPacienterDTO pacienterDTO = new RegistroPacienterDTO("Douglas", "douglas@gmail.com", "10012176594",
                "3107371692", "12345", "url_foto", LocalDate.of(2002, 7, 15), ciudad, tipoSangre, "Ninguna");

        int codigoPaciente;
        try {
            codigoPaciente = pacienteServicio.registrarse(pacienterDTO); // Guardamos el registro usando el método del
                                                                         // servicio
        } catch (Exception e) {
            throw new RuntimeException(e);
        }

        // Comprobamos que sí haya quedado guardado. Si se guardó debe retornar el
        // código (no 0).
    ...
    public void editarPerfil() throws Exception {
        ...
        DetallePacienteDTO modificado = new DetallePacienteDTO(enBase.codigo(), enBase.cedula(), enBase.nombre(),
                enBase.telefono(), enBase.urlFoto(), enBase.ciudad(), enBase.fechaNacimiento(), enBase.alergias(),
                enBase.eps(), enBase.tipoSangre(), enBase.correo());

        pacienteServicio.editarPerfil(1, modificado);
        DetallePacienteDTO actualizado = pacienteServicio.verDetallePaciente(1);
        Assertions.assertNotEquals(enBase.eps(), actualizado.eps()); // This assertion might fail if eps didn't change,
    ...
    public void eliminarCuentaTest() {
        ...
        try {
            ...
            detallePacienteDTO = pacienteServicio.verDetallePaciente(1); // Intenta obtener el detalle del paciente
    ...
    public void filtrarMedicoCita() {
        ...
        try {
            Especialidad especialidad = especialidadRepo.findById(1).orElseThrow(); // Assuming 1 exists
            medicoCitaDTOList = pacienteServicio.filtrarMedicoCita(especialidad, LocalDateTime.of(2023, 10, 20, 9, 0));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public void verDetallePaciente() throws Exception {
        ...
        Assertions.assertEquals(4, paciente.codigo()); // Assuming ID 1 maps to codigo 4? Or maybe this assertion is
